...
class GeckoCore(SignalCatch, GProcess):
    ...
    def handle_reply3(self, data, address):
        ret = None
        if data[1] == MsgsIPC.Pub:
            services[data[2]] = zmqTCP(*address)
            ret = (1234,)  # all ok
            self.perf.push(data[3], data[4])

        elif (data[1] == MsgsIPC.Sub) and (data[2] in services.keys()):
            ret = (data[2], services[data[2]],)
    ...
    def run(self):
        ...
        #     self.handle_reply2
        # )
        provider = BeaconServer('local', callback=self.handle_reply2)
        provider.start()
